...
**Internal:**
- `@/modules/player/domain/players`, `@/modules/hashimon/domain/hashimons`, `@/modules/mining/domain/mining` — routes are thin
  adapters; all business logic and DB access live there, not in `http/`.
- `@/modules/core/logger` — default pino instance; overridable per-app for tests.
