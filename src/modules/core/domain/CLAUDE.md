...
- `audit()` must be called with the transaction's `client`, not a bare `query()` — passing the wrong client silently writes the audit row outside the transaction, breaking the "commits atomically" guarantee every caller relies on. Callers: `hashimon::emit`, `mining::submitShare`, `mining::submitYield`, `payments::settleAndCredit` (via `applyWebhook`), `incubation::createLot` (lot open), `incubation::applyShare` (mark applied, and lot closed when full) and `incubation::closeLotById` (close and refund). `grep -rn "audit(" src` before changing its signature.
- The one ledger mutation that writes no audit row: the per-turn credit debit in `companion/domain/chat.ts::speak`.
