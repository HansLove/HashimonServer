...
- **A yield is decided on two axes, and only one of them is the hash.** `submitYield` recomputes the hash and requires `evaluateYield`'s window to clear its floor — that is the *event* (`no_yield` otherwise). The *tier* recorded is the place's: the zone of the player's town tower, or the consumable floor at `vault:<player>` when there is no town or no tower. The hash's own depth is kept only as `yield_bits` telemetry. Both are resolved server-side, so a tier is never a client claim.
