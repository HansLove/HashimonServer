...
- **foods.ts** — the food graph (`FOODS`): the named items a harvest can yield and their weights. `foodFor` picks the concrete item within the tier `submitYield` already decided, deterministically from the hash's material window; `from` only draws the tree, it gates nothing. Served whole by `GET /foods/catalog`.
...
- **Template caching.** `getPreparedTemplate` caches one `PreparedTemplate` in module memory for `config.templateRefreshMs`, so many jobs across many creatures reuse one `getblocktemplate` RPC round-trip; on a fetch error it logs (host only, no credentials) and returns the stale cached template rather than failing the caller — but only while that template is younger than `MAX_STALE_MS` (20 min, a hardcoded `ponytail:` constant). Past that ceiling a failed fetch returns `null`, and `issueJob` falls back to `'bound'` exactly as it does before the first successful fetch: mining against a long-superseded block is worse than no bitcoin-mode job.
...
- `btcNodeUrl` (config) — RPC URL with embedded basic-auth credentials for `block-template.ts`; wrong/missing → `getPreparedTemplate` fails silently and serves the last cached template (or `null` before first success, or once the cache is older than `MAX_STALE_MS`).
