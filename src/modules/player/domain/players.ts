...
import { isUniqueViolation, query, type QueryFn } from "@/modules/core/db/pool";
...
//`db` defaults to the real pool-backed query — every call site outside a test is
//unaffected. A test substitutes it with any function matching QueryFn, no live
//Postgres or module mocking required.
export async function findOrCreatePlayer(
  ...
  if (input.publicKey) {
    const existing = await db<Player>(`SELECT * FROM players WHERE public_key = $1`, [input.publicKey]);
    if (existing.rows[0]) { return { player: existing.rows[0], created: false }; }
  }
  const created = await db<Player>(
...
export async function getPlayer(id: string, db: QueryFn = query): Promise<Player | null> {
  const res = await db<Player>(`SELECT * FROM players WHERE id = $1`, [id]);
  return res.rows[0] ?? null;
}

export async function getPlayerByUsername(username: string, db: QueryFn = query): Promise<Player | null> {
...
export async function createSession(playerId: string, db: QueryFn = query): Promise<Session> {
  const token = randomBytes(32).toString("hex");
  const expires = new Date(Date.now() + config.sessionTtlHours * 3600 * 1000);
  const res = await db<Session>(
...
export async function playerForToken(token: string, db: QueryFn = query): Promise<Player | null> {
  const res = await db<Player>(
...
//Injectable collaborators for the registration/claim/rebirth flows. All optional,
//defaulting to the real implementations, so no production call site ever passes
//this — a test supplies fakes for whichever collaborator it needs to isolate,
//without mocking hashimon/affiliate's own DB-backed modules.
export interface PlayerDeps {
  db?: QueryFn;
  emitFn?: typeof emit;
  resolveAffiliateCodeFn?: typeof resolveAffiliateCode;
}
...
export async function registerOwner(input: {
  ...
  const { db = query, emitFn = emit, resolveAffiliateCodeFn = resolveAffiliateCode } = deps;
  ...
  const existing = await getPlayerByUsername(username, db);
  if (existing) {
    ...
    return claimLuantiGuest(existing, input, identity, deps);
  }

  const keyMaterial = await deriveOwnerKeyMaterial(input, db);
  ...
  const referredBy = await resolveAffiliateCodeFn(input.ref);
  enrich({ referred_by: referredBy, ref_supplied: Boolean(input.ref) });

  let player: Player;
  try {
    const inserted = await db<Player>(
      `INSERT INTO players (
  ...
  const { session, hashimon } = await emitStarterAndBindSession(player, identity, () =>
    db(`DELETE FROM players WHERE id = $1`, [player.id]), deps
  );
...
async function claimLuantiGuest(
  ...
  let player: Player;
  try {
    const claimedRow = await db<Player>(
      `UPDATE players
 ...
 *  the result. */
async function deriveOwnerKeyMaterial(
  input: {
    password: string;
    publicKey?: string;
    custody?: Custody;
  ...
  if (input.publicKey) {
    ...
    const keyTaken = await db(`SELECT 1 FROM players WHERE public_key = $1`, [input.publicKey]);
    if (keyTaken.rows[0]) {
      throw new AppError(409, "publicKey already registered", "public_key_taken");
    }
    return {
...
async function emitStarterAndBindSession(
  ...
  const { db = query, emitFn = emit } = deps;
  try {
    const row: HashimonRow = await emitFn({
      ownerId: player.id,
    ...
    const session = await createSession(player.id, db);
...
export async function loginOwner(username: string, password: string, db: QueryFn = query): Promise<{
  player: Player;
  session: Session;
  ...
  const player = await getPlayerByUsername(username.trim(), db);
 ...
 *  this password". */
export async function listLuantiAuthEntries(db: QueryFn = query): Promise<LuantiAuthEntry[]> {
  const res = await db<{ username: string; luanti_password: string; public_key: string | null }>(
 ...
 *  register on the web. */
export async function registerLuantiGuest(name: string, passwordEntry: string, db: QueryFn = query): Promise<Player> {
...
export async function claimSelfCustody(playerId: string, db: QueryFn = query): Promise<Player> {
  const res = await db<Player>(
    `UPDATE players
