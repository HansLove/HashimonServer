...
- **Byte order in `hashBitcoinJob` follows Stratum, not the block explorer.** `merkleBranch` entries and the merkle root fold raw, in internal (digest) byte order — nothing is flipped inside the loop; only `prevhashBE`, `versionHex`, `bits`, `nTimeHex` and `nonceHex` are display-order inputs the header reverses. `domain/block-template.ts::computeMerkleBranch` returns the branch in that same internal order, and `stratumPrevHashToBE` undoes the 4-byte word swab a pool applies to prevhash. The parity guard is the *external* golden vector in `core.test.ts` (spoon's own `SharePayload`, hash included) — a self round-trip cannot catch an orientation slip, because both sides slip together.
