...
**Estado:** documentación del sistema **tal como está implementado hoy**, verificado línea por línea contra el código real (`src/`, `genesis-portal/src/lib/`, `luanti/mods/`), no contra el whitepaper ni la memoria del proyecto. Este archivo se perdió al mover `docs/` dentro de `api/docs/` el 2026-08-20 y fue reconstruido el mismo día — si encuentras otra copia con contenido divergente, esta es la vigente.
...
1. **El ADN no es un hash arbitrario metido en un OP_RETURN.** Hoy, para todo nacimiento (Genesis incluido), es `SHA256(templateId : birthNonce : speciesKey)` con `birthNonce` generado por el servidor (`hashimon/domain/hashimons.ts::emit`). La fórmula `SHA256(ownerPublicKey : speciesKey : birthday)` de §6 está **decidida pero no implementada**: ningún código usa `ownerPublicKey` al derivar ADN. Lo que sí cambió para Genesis es de dónde sale `speciesKey`: lo fija la fecha de nacimiento (Birth Identity, `core/core/birth-identity.ts`), no la elección del jugador.
...
4. El sistema tiene **dos tipos de nacimiento** con requisitos de diseño distintos: **Genesis** (fijado por la fecha de nacimiento del jugador; en el diseño de §6, determinista y sin necesidad de anti-grind) y **Natural** (sembrado por un bloque de Bitcoin real). Más una tercera capa que **no es de emisión sino de renderizado**: los **Exceptions** (diseños 3D artesanales que no derivan del ADN). Modelo completo en §7 — **especificado, no implementado**.
...
### 1.1 Genesis (decidida, **no implementada**)

```typescript
// genesis-portal/src/lib/compiler.ts — compile()
...
`birthday` es opcional — si el jugador no lo da, se usa `""` fija: `SHA256(ownerPublicKey:speciesKey:"")`. Ver §6.2: el servidor todavía no deriva ningún ADN así.

### 1.2 Fórmula vigente en el código (`src/modules/core/core/dna.ts`, llamada desde `hashimons.ts::emit`)
...
```
ADN = SHA256( templateId : birthNonce : speciesKey )
```

`birthNonce` lo genera el servidor con `hashimons.ts::deriveBirthNonce`: HMAC-SHA256 con `BIRTH_SECRET` sobre entropía aleatoria + `ownerId:speciesKey:retry`, truncado a 16 hex, para que cada nacimiento sea auditable. Sólo si `BIRTH_SECRET` no está definido cae a 8 bytes aleatorios crudos. Aplica a Genesis y a no-Genesis por igual; en un Genesis V2, `speciesKey` y `templateId` salen de la Birth Identity.
...
- No se guarda el "look" derivado (tipo, color, arquetipo, stats escalados) en PostgreSQL — se recomputa on-demand desde `dna` + `speciesKey` cada vez que se muestra. Comentario explícito en `src/modules/core/db/schema.sql`: *"Stats and look are NOT stored: they are derived from dna + pow by the Caos Core."*
...
| | **Genesis** | **Natural** |
|---|---|---|
| Quién lo origina | La fecha de nacimiento del jugador (Birth Identity) | Un bloque de Bitcoin real, en checkpoints administrativos |
...
> **Estado: no implementada.** `emit()` sigue derivando todo Genesis con `SHA256(templateId:birthNonce:speciesKey)` y nonce de servidor (§1.2). Lo único ya vigente de este rediseño es que la especie Genesis la fija la fecha de nacimiento.
...
- `speciesKey` — la especie Genesis que fija la Birth Identity (`g2_<spirit>_<element>`); los `genesis_*` de V1 ya no se emiten.
...
### 6.4 Recomendación de seguimiento (parcialmente implementada)

`hashimons.algo_version` ya existe (NOT NULL, `emit()` lo llena con la constante `CORE_VERSION`; `config.algoVersion` / `ALGO_VERSION` no lo afecta) y registra la versión del Caos Core con la que nació cada fila. Falta un `birth_mode` que distinga qué regla de ADN aplicó, necesario el día que la fórmula de §6.2 conviva con la actual. Los Hashimon ya emitidos con la fórmula anterior no se ven afectados (su `dna` ya está guardado, no se recalcula).
...
- **Ya existe en el código.** `leadingZeroBits()` está en [`src/modules/core/core/pow.ts`](../src/modules/core/core/pow.ts) y es exactamente la misma función con la que evolucionan los Hashimon. Un solo concepto de "suerte" en todo el sistema.
...
**Esto ya está implementado**: es el registro `hashimon_media/<dna>.glb` de [`media.lua`](../../luanti/mods/hashimon_core/media.lua), el primer nivel de la cadena de spawn. No hay mecanismo nuevo que construir, solo acotarlo y etiquetarlo.
...
- El **arquetipo** sale de `DNA.pick(dna, 35, 2, ARCHETYPES)` en `genesis-portal/src/lib/compiler.ts` — las 5 especies Genesis dejan `species.archetype` sin definir, así que cada individuo saca su arquetipo de una posición de ADN independiente de su tipo. La lista de 16 arquetipos (`canine, feline, ursine, avian, aquatic, reptilian, arachnid, mollusk, humanoid, construct, celestial, spectral, fungal, crystalline, amorphous, hybrid`) ni siquiera incluye "roedor".
...
- [`genesis-portal/src/lib/compiler.ts`](../../genesis-portal/src/lib/compiler.ts) — `TYPES` reordenado 0-F; `ELEMENT_PALETTES` actualizado.
- [`luanti/mods/hashimon_core/dna_compiler.lua`](../../luanti/mods/hashimon_core/dna_compiler.lua) — mismo cambio de paleta.
- [`luanti/mods/hashimon_entities/entities.lua`](../../luanti/mods/hashimon_entities/entities.lua) — `TYPE_COLORS` ampliado (incluía `vegetal` faltante por completo).
...
### 8.2 Corrección aplicada 2026-08-20: catálogo muerto en `src/modules/hashimon/data/species.ts`
...
Verificado que **ningún código lee esos campos** — `emit()` en `src/modules/hashimon/domain/hashimons.ts` solo usa `species.templateId` y la existencia de la clave. Se limpió el archivo a solo `{ templateId }` por especie; el tipo/arquetipo/color de cada Hashimon sigue viniendo exclusivamente del compilador del portal + DNA, nunca de este archivo. Tests (`src/modules/core/core/core.test.ts`) y `tsc --noEmit` verificados sin regresión tras el cambio.
...
|---|---|
| [`src/modules/core/core/dna.ts`](../src/modules/core/core/dna.ts) | Fórmula histórica del ADN (`templateId:birthNonce:speciesKey`), helpers de lectura de nibbles |
| [`src/modules/hashimon/domain/hashimons.ts`](../src/modules/hashimon/domain/hashimons.ts) | `emit()` — nacimiento, generación de `birthNonce`, inserción con reintento |
| [`src/modules/hashimon/data/species.ts`](../src/modules/hashimon/data/species.ts) | Allowlist de emisión — solo `templateId`, sin tipo/arquetipo (corregido 2026-08-20) |
| [`src/modules/core/db/schema.sql`](../src/modules/core/db/schema.sql) | Restricción `UNIQUE` en `dna`, estructura de `players`/`hashimons`/`sessions` |
| [`src/modules/core/http/auth.ts`](../src/modules/core/http/auth.ts) | `requireSession` — el único mecanismo de autenticación real |
| [`src/modules/player/domain/players.ts`](../src/modules/player/domain/players.ts) | `loginOwner()` (contraseña vía argon2), validación de formato de `publicKey` |
| [`src/modules/core/core/pow.ts`](../src/modules/core/core/pow.ts) | `verifyStoredPow`, `verifyJobShare`, `leadingZeroBits` — verificación por recómputo y métrica de suerte usada en §7 |
| [`src/modules/mining/domain/block-template.ts`](../src/modules/mining/domain/block-template.ts) | Conexión al nodo Bitcoin (`getblocktemplate`), coinbase partido alrededor del extranonce — la misma conexión sirve para los datos históricos que necesita §7.1 |
| [`luanti/mods/hashimon_core/media.lua`](../../luanti/mods/hashimon_core/media.lua) | Registro `hashimon_media/<dna>.glb` — la capa de Exception (§7.4), ya implementada |
| [`genesis-portal/src/lib/compiler.ts`](../../genesis-portal/src/lib/compiler.ts) | `compile()` — ADN → look; `TYPES`/`ELEMENT_PALETTES` canónicos |
| [`genesis-portal/src/lib/species.ts`](../../genesis-portal/src/lib/species.ts) | Catálogo del portal — 5 especies Genesis, `archetype` sin fijar a propósito |
