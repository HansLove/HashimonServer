...
buying is gated by `requireSession` alone rather than `canOwn`. Incubation has its own five
(unsigned webhook, `assigned` inferred from the 202, sweep on read paths rather than a
schedule, that sweep being an unindexed scan on a polling endpoint, no reconciliation against
CaosEngine) — *Known gaps in the incubation flow*.
